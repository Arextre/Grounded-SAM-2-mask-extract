...
def load_video_frames_from_tensor(
    ...
    """Load the video frames from a tensor

    Args:
        video_tensor (torch.Tensor): shape like f * c * h * w

    The frames are resized to image_size x image_size and are loaded to GPU if
    `offload_video_to_cpu` is `False` and to CPU if `offload_video_to_cpu` is `True`.
    """
    if not isinstance(video_tensor, torch.Tensor):
        raise TypeError(f"video_tensor type error: {type(video_tensor)}")
    img_mean = torch.tensor(img_mean, dtype=torch.float32)[:, None, None]
    img_std = torch.tensor(img_std, dtype=torch.float32)[:, None, None]
    _, _, video_height, video_width = video_tensor.shape
    images = []
    # for frame_idx in range(len(video_tensor)):
    for frame_idx in tqdm(range(len(video_tensor)), desc="frame loading (TENSOR)"):
        resized_frame = F.interpolate(
            video_tensor[frame_idx].unsqueeze(0),
