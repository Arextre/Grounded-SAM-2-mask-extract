...
def save_video_mask_as_json(
    ...
    param:
        ...
        @annotations:
            ...
            each dict {
                "id": int, instance id
                "image_id": int, frame id of this mask
...
def load_video_as_tensor(
    ...
    Args:
        ...
        discard_interval (int): interval to discard some frames

    Returns:
        video_tensor (torch.Tensor): tensors of video
        frame_idx (list): the index of frames (if discarded some frames,
            use this to locate the index of frame)
    """
...
def get_mask_json(rank, json_save_folder, video_file_lists, args):    
    ...
    _grounding_model = (AutoModelForZeroShotObjectDetection
                        .from_pretrained(model_id).to(_device))
    ...
    for task_id in range(task_st, task_ed):
        ...
        # video_path = "/home/notebook/code/personal/S9060429/Grounded-SAM-2/notebooks/videos/bedroom.mp4"

        parent_dir = os.path.dirname(video_path)
        video_name = os.path.basename(video_path)
        video_name = os.path.splitext(video_name)[0]
        # frame_folder = os.path.join(parent_dir, video_name + "_frames")
        ...
        MASK_VISULIZATION_OPTION = True
        if MASK_VISULIZATION_OPTION:
            # visulize the mask to check
            mask_viz(video_tensor, video_segments, OBJECTS)

        """
        Step 5: Prepare the information of json files
        """
        categories = []
        annotations = []
        images = []
        ...
        save_video_mask_as_json(images, annotations, categories, json_save_path)
        if is_validate:
...
def launch_parallel_video_processing(
    ...
    # task list
    video_file_list = []
    video_folder = "/home/notebook/code/personal/S9060429/Grounded-SAM-2/notebooks/videos/mouth-closed"
    ...
    print(f"[Rank Main]: Found {task_count} files")
    ...
    start_time = time.time()
    get_mask_json(0, json_save_folder, undo_task_list, args_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--video_folder",
        type=str,
        default="./notebooks/videos/mouth-closed/"
        # required=True
    )
    parser.add_argument(
        "--json_save_folder",
        type=str,
        default="./notebooks/json_folder/"
        # required=True
    )
    parser.add_argument(
        "--start_idx",
